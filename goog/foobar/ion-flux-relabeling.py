...
Output:
    (int list) [21, 15, 29]
"""
# Do i even need -1 in setting left?
def helper(nodes, fc):
    left = nodes - 1 - ((nodes-1)/2)
    right = nodes - 1
    if left == fc or right == fc:
        return nodes
    else:
        if fc <= left:
            return helper(left, fc)
        else:
            return helper(right,fc)
...
def answer(h, q):
    ...
    for flux_converter in q:
        if 1 <= flux_converter < num_nodes:
            res.append(helper(num_nodes, flux_converter))
        else:
            res.append(-1)
    return res
